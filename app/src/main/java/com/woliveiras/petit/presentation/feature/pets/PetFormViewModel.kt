...
constructor(
  ...
  private val hasRestoredDraft: Boolean = savedStateHandle[KEY_HAS_DRAFT] ?: false
  ...
  private val _uiState =
    MutableStateFlow(
      PetFormUiState(
        isEditMode = hasRestoredDraft && petId != null,
        petId = petId.takeIf { hasRestoredDraft },
        petType =
          savedStateHandle.get<String>("breedPetType")?.let {
            runCatching { PetType.valueOf(it) }.getOrNull()
          } ?: PetType.OTHER,
        name = savedStateHandle[KEY_NAME] ?: "",
        birthDate = savedStateHandle.get<Long>(KEY_BIRTH_DATE)?.let(LocalDate::ofEpochDay),
        sex =
          savedStateHandle.get<String>(KEY_SEX)?.let { runCatching { Sex.valueOf(it) }.getOrNull() }
            ?: Sex.UNKNOWN,
        breed = savedStateHandle["breed"] ?: "",
        breedId = savedStateHandle["breedId"],
        breedDisplayName = savedStateHandle["breedDisplayName"],
        color = savedStateHandle[KEY_COLOR] ?: "",
        microchipNumber = savedStateHandle[KEY_MICROCHIP] ?: "",
        passportNumber = savedStateHandle[KEY_PASSPORT] ?: "",
        notes = savedStateHandle[KEY_NOTES] ?: "",
        photoUri = savedStateHandle[KEY_PHOTO_URI],
      )
    )
    ...
    viewModelScope.launch {
      ...
      try {
        ...
        if (pet != null) {
          ...
          _uiState.value =
            if (hasRestoredDraft) {
              _uiState.value.copy(isLoading = false, isEditMode = true, petId = pet.id)
            } else {
              _uiState.value.copy(
                isLoading = false,
                isEditMode = true,
          ...
          if (!hasRestoredDraft) persistBreedState(_uiState.value)
        }
      } catch (e: Exception) {
  ...
  fun updateName(name: String) {
    ...
    persistFormDraft(_uiState.value)
  }
  ...
  fun applyBreedSelection(selection: BreedSelectionValue) {
    _uiState.value =
      _uiState.value.copy(
        breed = selection.breed,
        breedId = selection.breedId,
        breedDisplayName = selection.displayName,
        breedError = null,
      )
    persistBreedState(_uiState.value)
    persistFormDraft(_uiState.value)
        ...
        val petToSave =
          if (state.isEditMode && state.petId != null) {
            ...
            existingPet?.copy(
              ...
              sex = state.sex,
              breed = preserveExactChangedText(state.breed, petLoadedForEdit?.breed),
            ...
            Pet(
              ...
              birthDate = state.birthDate,
              sex = state.sex,
              breed = state.breed.takeUnless { it.isBlank() },
              breedId = state.breedId,
              color = state.color.trim().ifBlank { null },
              microchipNumber = state.microchipNumber.trim().ifBlank { null },
              passportNumber = state.passportNumber.trim().ifBlank { null },
  ...
  private fun preserveExactChangedText(current: String, original: String?): String? =
    if (current == original.orEmpty()) original else current.takeUnless { it.isBlank() }
  ...
  private fun persistFormDraft(state: PetFormUiState) {
    savedStateHandle[KEY_HAS_DRAFT] = true
    savedStateHandle[KEY_NAME] = state.name
    savedStateHandle[KEY_BIRTH_DATE] = state.birthDate?.toEpochDay()
    savedStateHandle[KEY_SEX] = state.sex.name
    savedStateHandle[KEY_COLOR] = state.color
    savedStateHandle[KEY_MICROCHIP] = state.microchipNumber
    savedStateHandle[KEY_PASSPORT] = state.passportNumber
    savedStateHandle[KEY_NOTES] = state.notes
    savedStateHandle[KEY_PHOTO_URI] = state.photoUri
    persistBreedState(state)
  }

  private companion object {
    const val KEY_HAS_DRAFT = "petFormHasDraft"
    const val KEY_NAME = "petFormName"
    const val KEY_BIRTH_DATE = "petFormBirthDate"
    const val KEY_SEX = "petFormSex"
    const val KEY_COLOR = "petFormColor"
    const val KEY_MICROCHIP = "petFormMicrochip"
    const val KEY_PASSPORT = "petFormPassport"
    const val KEY_NOTES = "petFormNotes"
    const val KEY_PHOTO_URI = "petFormPhotoUri"
  }
}
